...
dp[i] =
\begin{cases}
dp[i] + dp[i - 1], & \text{0 < S[i] <= 9} \\
dp[i] + dp[i - 2], & \text{10 < S[i - 1] + S[i] <= 26}
\end{cases}
$$
...
up[i] =
\begin{cases}
down[i - 1] + 1, & \text{nums[i] $\gt$ nums[i - 1]} \\
up[i - 1], & \text{nums[i] $\le$ nums[i - 1]}
\end{cases}
\\
down[i] =
\begin{cases}
up[i - 1] + 1, & \text{nums[i] $\lt$ nums[i - 1]} \\
up[i - 1], & \text{nums[i] $\ge$ nums[i - 1]}
\end{cases}
$$
...
dp[i][j] = 
\begin{cases}
dp[i - 1][j - 1] + 1, & \text{S1[i] = S2[j]} \\
max(dp[i - 1][j], dp[i][j - 1]), & \text{S2[i] $\ne$ S2[j]}
\end{cases}
$$
代码实现：
...
得到状态转移方程：
$$
dp[k][i] = max(dp[k - 1][i], dp[k - 1][i - zeros[k]] + 1) \\
dp[k][j] = max(dp[k - 1][j], dp[k - 1][j - ones[k]] + 1)
$$
空间优化：
